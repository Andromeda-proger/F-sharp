...
let rec treefold (tree: BinaryTree) (value: string) (acc: int): int =
    match tree with
    | Node(data, left, right) ->
        let newAcc = if data = value then acc + 1 else acc // Увеличиваем аккумулятор, если элемент найден
        let leftAcc = treefold left value newAcc           // Рекурсивно обходим левое поддерево
        treefold right value leftAcc                      // Рекурсивно обходим правое поддерево
    | Empty -> acc                                        // Возвращаем аккумулятор для пустого дерева
...
let main _ =
    ...
    // Используем treefold для подсчета вхождений элемента
    let acc = treefold binTree N 0

    // Проверяем значение аккумулятора
    if acc > 0 then
        printfn "Элемент '%s' найден в дереве." N
    else
        printfn "Элемент '%s' не найден в дереве." N

    0 // Возвращаем 0 как код успешного завершения программы